...
<head>
    ...
    <script>
        ...
        class = "btn btn-primary" > Chambre < /a>
...
<body class="bg-white dark:bg-[#0a0a0a] text-slate-900 dark:text-slate-100 antialiased font-sans hero-gradient">
    ...
    <header class="fixed top-0 left-0 right-0 z-50 glass-effect border-b border-slate-200/60 dark:border-white/10">
        <nav class="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3 cursor-pointer">
                <svg class="w-6 h-6 text-primary dark:text-white" viewBox="0 0 24 24" fill="none"
            ...
            <div
                class="hidden lg:flex items-center gap-10 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
            ...
            <div class="flex items-center gap-4">
                @auth
                    ...
                    <div class="flex items-center gap-3">
                        <div
                            class="flex items-center gap-3 pl-4 pr-2 py-1.5 border border-slate-200 dark:border-white/10 rounded-full bg-slate-50/50 dark:bg-white/5">
                            <span
                                class="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-600 dark:text-slate-300">
                                {{ Auth::user()->name }}
                            </span>
                            <div
                                class="w-7 h-7 rounded-full bg-primary dark:bg-indigo-500 flex items-center justify-center text-[10px] text-white font-bold">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="p-2 text-slate-400 hover:text-red-500 transition-colors group"
                                title="Logout">
                ...
                @else
                    ...
                    <div class="flex items-center gap-6">
                        <a href="{{ route('login') }}"
                            class="text-[11px] font-bold uppercase tracking-widest hover:text-primary dark:hover:text-white transition-all text-slate-500">
                            Login
                        </a>
                        <button
                            class="bg-primary dark:bg-white text-white dark:text-black px-6 py-2.5 rounded-sm text-[11px] font-bold uppercase tracking-widest transition-all hover:bg-slate-800 dark:hover:bg-slate-200 shadow-sm">
                            Book Now
                        </button>
                    </div>
                @endauth
    ...
    <main class="pt-40">
        ...
        <section class="max-w-7xl mx-auto px-8 mb-24">
            <div class="text-center max-w-4xl mx-auto mb-16">
                ...
                <h1
                    class="text-5xl md:text-7xl font-bold tracking-tight mb-8 leading-[1.1] text-slate-900 dark:text-white">
                    Refined stays for the <br><span class="italic font-normal text-slate-400">discerning</span>
                    traveler.
                </h1>
                <p class="text-lg text-slate-500 dark:text-slate-400 leading-relaxed max-w-2xl mx-auto">
                    A curated portfolio of the world's most exceptional hotels, villas, and retreats. We bridge the gap
            ...
            @auth
                <div class="max-w-6xl mx-auto">
                    <div
                        class="bg-white dark:bg-zinc-900 rounded-2xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] border border-slate-200 dark:border-white/5 flex flex-col md:flex-row items-stretch p-2">
                        <div
                            class="flex-[1.5] flex flex-col justify-center px-8 py-5 border-b md:border-b-0 md:border-r border-slate-100 dark:border-white/5">
                            <span
                                class="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1.5">Destination</span>
                            <input type="text" placeholder="Search cities or hotels"
                                class="bg-transparent focus:outline-none placeholder:text-slate-300 font-semibold text-sm text-slate-900 dark:text-white">
                        </div>
                        <div
                            class="flex-1 flex flex-col justify-center px-8 py-5 border-b md:border-b-0 md:border-r border-slate-100 dark:border-white/5">
                            <span class="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1.5">Check
                                In</span>
                        ...
                        <div class="flex-1 flex flex-col justify-center px-8 py-5">
                            <span
                                class="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1.5">Guests</span>
                            <input type="text" placeholder="2 Guests"
                                class="bg-transparent focus:outline-none placeholder:text-slate-300 font-semibold text-sm text-slate-900 dark:text-white">
                        </div>
                        <button
                            class="bg-primary dark:bg-white text-white dark:text-black px-10 py-5 md:py-0 rounded-xl font-bold text-[11px] uppercase tracking-widest transition-all hover:brightness-110 active:scale-[0.98]">
                            Search
                        </button>
                    </div>
                </div>
            @else
                <div class="text-center">
                    <a href="{{ route('login') }}"
                        class="inline-flex items-center gap-3 text-xs font-bold uppercase tracking-[0.2em] text-slate-400 hover:text-primary transition-colors">
        ...
        <section class="max-w-7xl mx-auto px-8 py-12 border-y border-slate-100 dark:border-white/5 mb-24">
            <div
                class="flex flex-wrap items-center justify-center md:justify-between gap-12 opacity-40 grayscale dark:invert">
        ...
        <section class="max-w-7xl mx-auto px-8 mb-32">
            <div class="flex items-end justify-between mb-12">
                <div>
                    <span class="text-[11px] font-bold uppercase tracking-[0.3em] text-indigo-600 mb-3 block">Curation
                        01</span>
                    <h2 class="text-4xl font-bold tracking-tight">The Season's Best</h2>
                </div>
                <button
                    class="group flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest border-b border-slate-200 dark:border-white/10 pb-2 hover:border-primary dark:hover:border-white transition-all">
                    Explore All Hotels
        ...
        <section
            class="max-w-7xl mx-auto px-8 py-24 bg-slate-50 dark:bg-zinc-900/40 rounded-3xl mb-32 border border-slate-100 dark:border-white/5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-20">
                <div>
                    <span class="text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-600 mb-4 block">Our
                        Philosophy</span>
                    <h2 class="text-4xl font-bold leading-tight mb-8">Redefining the standard of booking luxury.</h2>
                    <p class="text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">We don't just list hotels; we
                        vet them. Every property in our collection is hand-selected by our travel experts based on
                        design, service, and soul.</p>
                    <button
                        class="bg-primary dark:bg-white text-white dark:text-black px-8 py-4 rounded-sm text-[11px] font-bold uppercase tracking-widest transition-all hover:bg-slate-800">Learn
                        About Our Process</button>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-10">
                    <div>
                        <h4 class="font-bold uppercase tracking-widest text-[10px] mb-3">Expert Curation</h4>
                        <p class="text-sm text-slate-500">Only the top 1% of luxury properties make it through our
                            vetting process.</p>
                    </div>
                    <div>
                        <h4 class="font-bold uppercase tracking-widest text-[10px] mb-3">Global Concierge</h4>
                        <p class="text-sm text-slate-500">24/7 dedicated support for every booking, ensuring a seamless
                            experience.</p>
                    </div>
                    <div>
                        <h4 class="font-bold uppercase tracking-widest text-[10px] mb-3">Exclusive Perks</h4>
                        <p class="text-sm text-slate-500">Member-only upgrades, breakfast credits, and early check-ins.
                        </p>
                    </div>
                    <div>
                        <h4 class="font-bold uppercase tracking-widest text-[10px] mb-3">Seamless UX</h4>
                        <p class="text-sm text-slate-500">The most intuitive interface in the luxury travel industry.
                        </p>
                    </div>
                </div>
            </div>
    ...
    <footer class="bg-white dark:bg-[#0a0a0a] border-t border-slate-200 dark:border-white/10 pt-24 pb-12">
        <div class="max-w-7xl mx-auto px-8">
                ...
                <div class="col-span-1 md:col-span-2">
                    <div class="flex items-center gap-3 mb-8">
                        <div class="w-8 h-8 bg-primary dark:bg-white rounded flex items-center justify-center">
                            <svg class="w-5 h-5 text-white dark:text-black" viewBox="0 0 24 24" fill="none"
                    ...
                    <p class="text-slate-500 dark:text-slate-400 max-w-sm mb-8 leading-relaxed text-sm">
                        StayEasy is a global travel platform dedicated to providing access to the most refined
                        properties on earth.
                    </p>
                </div>

                <div>
                    <h5 class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-900 dark:text-white mb-8">
                        Company</h5>
            ...
            <div
                class="pt-8 border-t border-slate-100 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
